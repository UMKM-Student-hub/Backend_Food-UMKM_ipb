...
class ReviewService:
    ...
    async def submit_review(self, buyer_id: int, request: ReviewCreateRequest) -> Review:
        ...
        if order.status != OrderStatus.DONE:
            raise BusinessRuleViolationError("Ulasan hanya dapat diberikan untuk pesanan yang sudah selesai.")
        ...
        is_reviewed = await self._review_repo.exists_by_order_item(order.id, request.menu_item_id)
        if is_reviewed:
            raise BusinessRuleViolationError("Anda sudah memberikan ulasan untuk item menu ini.")
