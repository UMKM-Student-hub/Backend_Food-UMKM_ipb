...
class OrderService:
    ...
    async def place_order(
        ...
        if not umkm:
            raise NotFoundError(f"Toko dengan ID {umkm_id} tidak ditemukan.")

        if not umkm.can_accept_order():
            raise BusinessRuleViolationError(
                f"Maaf, {umkm.name} sedang tutup dan tidak menerima pesanan saat ini."
            )
        ...
        for req_item in parsed_items:
            ...
            if not menu_item_id or not quantity or quantity <= 0:
                raise BusinessRuleViolationError(
                    "Data item pesanan tidak lengkap atau tidak valid."
                )
    ...
    async def mark_order_done(self, order_id: int, buyer_id: int) -> Order:
        ...
        if order.buyer_id != buyer_id:
            raise PermissionDeniedError(
                "Hanya pemesan asli yang dapat menyelesaikan pesanan ini."
            )
